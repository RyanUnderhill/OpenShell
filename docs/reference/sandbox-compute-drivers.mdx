...
the completed process identity, and uses the kernel to validate access. The
final supervisor must match the probe's normalized identity attestation,
including when the image supplies the default process policy. Podman captures
a bounded, sanitized failure diagnostic, removes the probe, and only then
creates the final volume-backed sandbox. The resolved workspace is the cwd and
